   ...
   $code=trim($_POST["yzm"]);
    ...
    else
    {
        ...
        else
        {
			...
			{
			echo( "<script>alert('未输入验证码或验证码错误'); parent.location.href='user.php?act=register';</script>");
		}
		unset($_SESSION['code']);
...
elseif ($action == 'send_pwd_email')
{
   ...
   $code=trim($_POST["yzm"]);
		...
		if(strlen($code)>1 and $code==$y)
			{
		 $smarty->assign('uid',    $user_name);
        $smarty->assign('action', 'reset_password');
        $smarty->display('user_passport.dwt');
			}
			else {
			   show_message($_LANG['edit_yzm_failure'], $_LANG['back_page_up'], '', 'info');
			}
...
//	$k=md5(md5('$new_password').$ec_salt);
  //  $user_id  = isset($_POST['uid1'])  ? intval($_POST['uid1']) : $user_id;
    $user_id = $_POST['uid'];
   ...
   $sql="SELECT ec_salt FROM  ". $ecs->table('users') ."  WHERE  user_name = '" .$user_id."'";
    $ec_salt =$db->getOne($sql);
   
	if(!empty($ec_salt))	
 		{	
		  $sql="UPDATE ".$ecs->table('users'). " SET  password='".md5(md5($new_password).$ec_salt)."' WHERE user_name= '".$user_id."'";
			$db->query($sql);
            $user->logout();
            show_message($_LANG['edit_password_success'], $_LANG['relogin_lnk'], 'user.php?act=login', 'info');
		}
		else{
		$sql="UPDATE ".$ecs->table('users'). " SET  password='".md5($new_password)."' WHERE user_name= '".$user_id."'";
			$db->query($sql);
            $user->logout();
            show_message($_LANG['edit_password_success'], $_LANG['relogin_lnk'], 'user.php?act=login', 'info');
		}
