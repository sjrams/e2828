...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    else 
    {
     ...
     $sql = 'SELECT article_id, click_count,title,description,content, author, add_time, file_url,cat_id, open_type' .
               ' FROM ' .$GLOBALS['ecs']->table('article') .
	  ...
	  $rew = $GLOBALS['db']->getAll($sql);
	   foreach ($rew AS $key => $val)
    {
	 $temp_key = $key + 1;
	 if (20 == $rew[$key]['article_id'])
        {
            $rew[$temp_key]['selected'] = 1;
        }
        else
        {
            $rew[$temp_key]['selected'] = 0;
        }
	}
