        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            ...
            foreach ($attr_value_list as $attr_value => $info)
            {
				...
				if(mysql_query($sql)){
					...
					if($row['shop_price']>$info[attr_price] && $info[attr_price]!=0){
						$sql= "UPDATE " .$ecs->table('goods'). " SET shop_price = '$info[attr_price]' WHERE goods_id = '$goods_id' ";
						}
					}
                ...
                elseif ($info['sign'] == 'update')
                {
				  ...
				  if(mysql_query($sql)){
					...
					if($z>=$info[attr_price]   && $info[attr_price]!=0){
						$sql= "UPDATE " .$ecs->table('goods'). " SET shop_price = '$info[attr_price]' WHERE goods_id = '$goods_id' ";
						$z=$info[attr_price];
						}	
					}
